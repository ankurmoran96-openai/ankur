...
export function Hero() {
  ...
  return (
    <section className="w-full flex flex-col items-center relative overflow-hidden bg-transparent">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 space-y-24 md:space-y-32 pb-24 md:pb-40 relative z-10">
        <motion.div 
          initial="hidden" 
          whileInView="visible" 
          viewport={{ once: true }} 
        ...
        >
          {profileInfo.map((info, i) => (
            <Card key={i} className="glass-card bg-white/[0.02] border-white/5 p-4 md:p-6 flex items-center gap-3 md:gap-4 group">
              ...
              <div>
                <p className="text-[8px] md:text-[10px] font-code text-muted-foreground uppercase tracking-widest">{info.label}</p>
                <p className="text-sm md:text-xl font-display font-bold uppercase">{info.value}</p>
              </div>
            </Card>
          ))}
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 md:gap-20">
          ...
          >
            <div className="space-y-6">
              <h3 className="text-xl md:text-3xl font-display font-bold uppercase tracking-tight flex items-center gap-4">
                <Zap className="text-primary w-5 h-5 md:w-8 md:h-8" />
                My Passion
              </h3>
              <div className="grid gap-4 md:gap-6">
                {passions.map((p, i) => (
                  <Card key={i} className="glass-card bg-white/[0.02] border-white/5 p-5 md:p-8 space-y-3 md:space-y-4 group hover:border-primary/40">
                    <div className="flex items-center gap-3 md:gap-4 text-primary">
                      ...
                      <span className="text-lg md:text-2xl font-headline font-bold uppercase tracking-wide">{p.title}</span>
                    </div>
          ...
          >
            <div className="space-y-6 md:space-y-8">
              ...
              <div className="flex flex-wrap gap-2 md:gap-3">
                {workFocus.map((focus, i) => (
                  <Badge key={i} variant="outline" className="border-primary/20 bg-primary/5 text-primary text-[8px] md:text-xs py-1.5 md:py-2.5 px-3 md:px-5 rounded-none uppercase font-code tracking-widest hover:bg-primary hover:text-background transition-all duration-300">
                    {focus}
                  </Badge>
                ))}
              </div>
            ...
            <div className="p-6 md:p-10 rounded-3xl bg-primary/5 border border-primary/20 relative overflow-hidden group">
              <BrainCircuit className="absolute -bottom-8 -right-8 md:-bottom-10 md:-right-10 w-24 h-24 md:w-40 md:h-40 text-primary/5 group-hover:text-primary/10 transition-all duration-700" />
              <h4 className="text-lg md:text-2xl font-headline font-bold uppercase mb-3 md:mb-4 text-primary">Mission Directive</h4>
              <p className="text-sm md:text-lg text-muted-foreground font-light leading-relaxed italic">
                I am leveraging 4+ years of mobile-first AI engineering to bridge the gap between imagination and execution.
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <motion.div 
          ...
          viewport={{ once: true }} 
          variants={fadeUp}
          className="flex flex-col items-center gap-8 md:gap-12 pt-8 md:pt-12"
        >
          ...
          <Link href="/ai" className="group">
            <Button size="lg" className="h-16 md:h-24 px-8 md:px-16 glass-card bg-primary/5 hover:bg-primary hover:text-background border-primary/20 rounded-full transition-all duration-300 flex items-center gap-4 md:gap-8 group shadow-[0_0_30px_rgba(255,59,59,0.1)]">
              <div className="text-left">
                <p className="text-[7px] md:text-[10px] font-code tracking-[0.4em] opacity-60 mb-1 uppercase">Node 01</p>
                <p className="text-lg md:text-4xl font-display font-bold tracking-tighter uppercase">AI Arsenal</p>
              </div>
