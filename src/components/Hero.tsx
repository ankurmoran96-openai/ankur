...
export function Hero() {
  const profileInfo = [
    ...
    { icon: MapPin, label: "Location", value: "Assam" },
    { icon: User, label: "Status", value: "Online" },
  ];

  const passions = [
    { icon: Mic2, title: "Vocalist", desc: "Consistently mastering harmonies and vocal expression." },
    { icon: Code2, title: "Vibe-coding", desc: "High-quality AI orchestration directly from mobile." },
  ];

  const workFocus = [
    "AI Integration", "AI/ML", "Applied Engineering", "Prompt Engineering", 
    "AI Architecture", "Python", "MLOps", "AI AGENTS", "Web", "Linux Scripts"
  ];
  ...
  return (
    <section className="w-full flex flex-col items-center relative overflow-hidden bg-transparent">
      <HeroGeometric 
        badge="System Online | Tactical V4.1"
        title1="ANKUR"
        title2="MORAN"
      />
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-6 space-y-12 md:space-y-32 pb-24 md:pb-40 relative z-10">
        ...
        >
          {profileInfo.map((info, i) => (
            <Card key={i} className="glass-card p-3 md:p-6 flex items-center gap-2 md:gap-4 group border-white/5">
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-20">
          ...
          >
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="grid gap-3 md:gap-4">
                {passions.map((p, i) => (
                  <Card key={i} className="glass-card p-4 md:p-8 space-y-2 group border-white/5">
          ...
          >
            <div className="space-y-4 md:space-y-8">
              ...
              <div className="flex flex-wrap gap-2">
                {workFocus.map((focus, i) => (
                  <Badge key={i} variant="outline" className="border-primary/20 bg-primary/5 text-primary text-[8px] md:text-xs py-1.5 md:py-2 px-2 md:px-4 rounded-none uppercase font-code tracking-widest hover:bg-primary hover:text-background transition-colors">
                    {focus}
                  </Badge>
                ))}
              </div>
            ...
            <div className="p-5 md:p-10 rounded-2xl bg-primary/5 border border-primary/20 relative overflow-hidden group">
              ...
              <h4 className="text-sm md:text-2xl font-headline font-bold uppercase mb-2 text-primary leading-none">Mission Directive</h4>
              <p className="text-xs md:text-lg text-muted-foreground font-light leading-relaxed italic">
                Bridging the gap between imagination and execution with mobile-first AI engineering and cloud orchestration.
              </p>
            </div>
          </motion.div>
        </div>
        ...
        >
          <div className="w-full flex items-center gap-4 md:gap-8">
            ...
            <span className="text-[10px] font-code text-primary tracking-[0.4em] uppercase text-center whitespace-nowrap">Tactical Access</span>
            <div className="h-px flex-1 bg-gradient-to-l from-transparent to-primary/20" />
          </div>

          <Link href="/ai" className="group" prefetch>
            <Button size="lg" className="h-14 md:h-24 px-6 md:px-16 glass-card bg-primary/5 hover:bg-primary hover:text-background border-primary/20 rounded-full transition-all flex items-center gap-4 md:gap-8 shadow-[0_0_30px_rgba(255,59,59,0.1)]">
              <div className="text-left">
                <p className="text-[8px] md:text-[10px] font-code tracking-[0.4em] opacity-60 mb-0.5 uppercase">Node 01</p>
                <p className="text-base md:text-4xl font-display font-bold tracking-tighter uppercase leading-none">AI Arsenal</p>
              </div>
