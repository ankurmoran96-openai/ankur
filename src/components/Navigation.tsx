      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <Sheet>
          ...
          <SheetContent side="right" className="w-full sm:max-w-md bg-background/95 backdrop-blur-2xl border-l border-primary/20 p-0 overflow-hidden flex flex-col">
            <div className="p-12 flex-1 flex flex-col justify-center space-y-12">
               <div className="space-y-4">
                 <p className="text-primary font-code text-xs tracking-[0.4em] uppercase opacity-60">Navigation Nodes</p>
                 <h2 className="text-4xl font-display font-bold tracking-tighter">DIRECTORY</h2>
               </div>
               ...
               <div className="space-y-4">
                 {navLinks.map((link) => (
                   <Link
                     ...
                     href={link.href}
                     className={cn(
                       "group flex items-center justify-between p-6 rounded-2xl border transition-all duration-500",
                       pathname === link.href 
                   ...
                   >
                     <div className="flex items-center gap-6">
                       <link.icon className={cn("w-8 h-8", pathname === link.href ? "text-background" : "text-primary")} />
                       <div>
                         <p className="text-xl font-headline font-bold uppercase tracking-tight">{link.name}</p>
                         <p className={cn("text-xs font-code tracking-widest uppercase opacity-60", pathname === link.href ? "text-background" : "text-muted-foreground")}>{link.desc}</p>
                       </div>
                     </div>
                     <ChevronRight className={cn("w-5 h-5 group-hover:translate-x-2 transition-transform", pathname === link.href ? "text-background" : "text-primary")} />
                   </Link>
                 ))}
               </div>
            </div>
            
            <div className="p-12 border-t border-primary/10 bg-primary/5">
